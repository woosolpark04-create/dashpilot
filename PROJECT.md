...
8. **Phase 7 — Deployment & Portfolio Readiness**
   ...
   - [x] **Phase 7C — Production Deployment and Portfolio Finalization — complete.** Readiness work (real screenshots, `NEXT_PUBLIC_APP_URL`/no-hardcoded-localhost, GitHub/Vercel readiness review, documented Supabase redirect URLs, a production QA plan, portfolio copy) was followed by the actual deployment: repo linked to the existing `dashpilot` Vercel project (team `Sparkclean`), all four production env vars confirmed/configured (`NEXT_PUBLIC_SUPABASE_URL`, `NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY`, `SUPABASE_SECRET_KEY`, `NEXT_PUBLIC_APP_URL=https://dashpilot-sigma.vercel.app`), and the full production QA plan below executed against the live deployment (see Decisions Log for the pass/fail detail and the one finding, documented in **Known Issues**). Production URL: **https://dashpilot-sigma.vercel.app**.
...
- **2026-09-22** — Phase 7C completed: production deployment and QA. Repo linked to the existing `sparkclean/dashpilot` Vercel project (no new project created); production env vars confirmed (`NEXT_PUBLIC_SUPABASE_URL`, `NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY`, `SUPABASE_SECRET_KEY`) and `NEXT_PUBLIC_APP_URL` set to `https://dashpilot-sigma.vercel.app`. The full production QA plan above was executed live via Playwright against that URL: login (valid/invalid), Overview, Users list/search/filter/detail, Invite dialog (validation only — no real invite sent), Leads list/search/filter/detail/create/edit/delete (against one fictional throwaway lead, created and removed, all 25 seed leads untouched), Settings, unauthenticated-access redirects, mobile (375×812) and desktop (1440px) layouts, `/auth/callback` with no params, absence of hardcoded localhost URLs, and absence of exposed secrets — all passed. One finding, a production-only React hydration warning on non-self `/users/[id]` pages, was diagnosed but not fixed (see **Known Issues**) since it doesn't reproduce locally and has no observed functional/visual impact, so a fix couldn't be authored and verified responsibly. `.gitignore` also updated to ignore `.vercel` and `.env*` (Vercel CLI's own local files), committed separately.

## Known Issues

- **Production-only React hydration warning (`#418`) on some `/users/[id]` pages.** Visiting another user's detail page (any account other than the signed-in admin's own) throws a client-side React hydration error in the browser console on the live Vercel deployment. It does **not** reproduce in `next dev`, nor in a local production build (`next build && next start`, including under artificial network latency) — the RSC data payload was confirmed byte-identical between local and production, and no server-side error is logged for the request. It has **no observed visual or functional impact**: the correct data renders every time (React recovers from a hydration mismatch by discarding and re-rendering the affected subtree client-side). No speculative fix was applied, since the cause couldn't be reproduced outside the live Vercel environment and a fix couldn't be verified. Worth a closer look via Vercel's own request tracing if it recurs or blocks anything in practice.
