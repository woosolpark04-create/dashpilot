...
- **Redirect URL**: `redirectTo` is built from `NEXT_PUBLIC_SITE_URL` (new, optional, public env var) + `/login`, defaulting to `http://localhost:3000/login` for local dev. **For production**, set `NEXT_PUBLIC_SITE_URL` to the deployed domain, and add that same redirect URL to the Supabase Dashboard → Authentication → URL Configuration → Redirect URLs allow-list — Supabase silently ignores/rejects a `redirectTo` that isn't allow-listed there. Note: there's no dedicated "accept invite / set password" page yet, so today the email link lands on `/login` with auth tokens in the URL — building that acceptance page is future work, out of scope for this phase.
