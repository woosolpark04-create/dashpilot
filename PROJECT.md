...
- **Redirect URL**: `redirectTo` is built from `NEXT_PUBLIC_SITE_URL` (new, optional, public env var) + `/auth/callback` (updated in Phase 4C — was `/login`), defaulting to `http://localhost:3000/auth/callback` for local dev. **For production**, set `NEXT_PUBLIC_SITE_URL` to the deployed domain, and add that same redirect URL to the Supabase Dashboard → Authentication → URL Configuration → Redirect URLs allow-list — Supabase silently ignores/rejects a `redirectTo` that isn't allow-listed there.
...
8. **Phase 7 — Deployment & Portfolio Readiness**
   ...
   - [x] **Phase 7B — Portfolio Documentation** — complete. Added `README.md` (overview, features, tech stack, architecture, security highlights, demo-data disclosure, dev setup, screenshot placeholders, project status, and a copy-ready portfolio summary). Screenshots themselves are explicitly left as TODOs — no image paths were invented.
   - [ ] Deploy to Vercel with a demo Supabase instance, take the actual screenshots for the README, and complete production QA.
